...
def test_csv_file_creation(tmp_path) -> None:
    """Verify CSV file is created."""

    output_file = tmp_path / "customers.csv"

    CsvWriter.write([create_customer()], output_file)

    assert output_file.exists()


def test_csv_contains_data(tmp_path) -> None:
    ...
    assert len(rows) == 1
    assert rows[0]["customer_id"] == "CUST-2026-00000001"
    assert rows[0]["first_name"] == "John"


def test_csv_empty_list(tmp_path) -> None:
